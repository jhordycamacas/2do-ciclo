...
public class Metodos {
    ...
    public Nodo insertar(Nodo actual, int valor, int i, int j) {
        ...
        if (valor < actual.valor) {
            actual.izq = insertar(actual.izq, valor, i, j);
        } else if (valor > actual.valor) {
            actual.der = insertar(actual.der, valor, i, j);
        }
    ...
    public void cambiarDatos() {
        ...
        for (int i = 0; i < matriz.length; i++) {
            for (int j = 0; j < matriz[0].length; j++) {
                raiz = insertar(raiz, matriz[i][j], i, j);
            }
        }
